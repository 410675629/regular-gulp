...
var ResetPwd = Component.extend({
    ...
    __getCurrentStatus:function(){
        ...
        if(!_.isEmptyObject(params)){
            ...
            if(this.data.currentStatus == 3){
                cacheService.getUserInfo(function(data,result){
                    if(data == '6'){
                        self.data.currentStatus = 6;
                    } else {
                        self.data.passport = data.passport;
                        self.data.idNumber = data.idNumber.slice(0,7) +'********' + data.idNumber.slice(14);
    ...
    __watchAll:function(){
        //当重新输入登录名时 取消错误信息
        var self = this;
        this.$watch('formData.passport',function(newV,oldV){
            if(newV != '' && newV.trim()!=''){
                self.data.errInfo = '';
                self.data.firstAllFlag.nameFlag = true;

            }else{
                self.data.firstAllFlag.nameFlag = false;
            }
            self.__setBtn('firstAllFlag','firstBtnFlag');
            self.$update();
        });

        this.$watch('formData.idNumber',function(newV,oldV){
            if(newV && newV.trim()!=''){
                self.data.errInfo = '';
                self.data.firstAllFlag.idFlag = true;

            }else{
                self.data.firstAllFlag.idFlag = false;
            }
            self.__setBtn('firstAllFlag','firstBtnFlag');
            self.$update();
        });
    ...
    submit1:function(){
        ...
        cacheService.sendResetPwdEmail(this.data.formData,function(data,result){
            ...
            for(var item in self.data.firstAllFlag ){
                self.data.firstAllFlag[item] = false;
            }
            self.__setBtn('firstAllFlag','firstBtnFlag');
